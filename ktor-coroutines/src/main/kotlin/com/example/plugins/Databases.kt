...
fun Application.configureDatabases() {
    ...
    val articleService = ArticleService(mongoDatabase)
    routing {
        get("/api/articles/{id}") {
            val id = call.parameters["id"] ?: throw IllegalArgumentException("No ID found")
            articleService.getById(id)?.let { car ->
                call.respond(car)
            } ?: call.respond(HttpStatusCode.NotFound)
        }
        get("/api/articles/") {
            call.respond(articleService.list())
        }
    }
}
...
fun Application.connectToMongoDB(): MongoDatabase {
    ...
    val host = environment.config.tryGetString("db.mongo.host") ?: "192.168.122.61"
    val port = environment.config.tryGetString("db.mongo.port") ?: "27017"
    val maxPoolSize = environment.config.tryGetString("db.mongo.maxPoolSize")?.toInt() ?: 20
    val databaseName = environment.config.tryGetString("db.mongo.database.name") ?: "pages"
